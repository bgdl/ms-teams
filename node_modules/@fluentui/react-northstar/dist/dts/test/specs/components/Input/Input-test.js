...
var htmlInputAttrs = ['id', 'name', 'pattern', 'placeholder', 'type', 'value'];
var getInputAttrsObject = function (value) {
    return htmlInputAttrs.reduce(function (acc, attr) {
        acc[attr] = value;
        return acc;
    }, {});
};
...
describe('Input', function () {
    commonTests_1.isConformant(Input_1.default, {
        eventTargets: {
            onChange: 'input',
            onKeyDown: 'input',
            onKeyPress: 'input',
    ...
    commonTests_1.implementsShorthandProp(Input_1.default)('input', Box_1.default, { mapsValueToProp: 'type' });
    describe('wrapper', function () {
        commonTests_1.implementsShorthandProp(Input_1.default)('wrapper', Box_1.default, { mapsValueToProp: 'children' });
        commonTests_1.implementsWrapperProp(Input_1.default, { wrapppedComponentSelector: 'input' });
    });
    it('renders a text <input> by default', function () {
        var inputComp = utils_1.mountWithProvider(React.createElement(Input_1.default, null));
        expect(inputComp.find('input[type="text"]').length).toBeGreaterThan(0);
    });
    describe('input related HTML attribute', function () {
        var inputAttrsObject = getInputAttrsObject(testValue);
        var domNode = getInputDomNode(utils_1.mountWithProvider(React.createElement(Input_1.default, tslib_1.__assign({}, inputAttrsObject))));
        htmlInputAttrs.forEach(function (attr) {
    ...
    describe('clearable', function () {
        it('calls onChange on Clearable icon click with an `empty` value', function () {
            ...
            var wrapper = utils_1.mountWithProvider(React.createElement(Input_1.default, { clearable: true, defaultValue: faker.lorem.word(), onChange: onChange }));
            wrapper
                .find("." + Input_1.default.slotClassNames.icon)
                .first()
                .simulate('click');
        ...
        it('does not call onChange and does not stop propagation if is already empty', function () {
            ...
            var stopPropagation = jest.fn();
            var nativeEventStopPropagation = jest.fn();
            var wrapper = utils_1.mountWithProvider(React.createElement(Input_1.default, { clearable: true, defaultValue: '', onChange: onChange }));
            wrapper.find('input').simulate('keydown', {
                keyCode: keyboardKey.Escape,
                key: 'Escape',
                stopPropagation: stopPropagation,
    ...
    describe('icon', function () {
        ...
        it('creates the Icon component when the icon shorthand is provided', function () {
            var inputComp = utils_1.mountWithProvider(React.createElement(Input_1.default, { icon: React.createElement(SearchIcon, null) }));
        ...
        it('creates an empty Icon component when the clearable prop is provided and the input has content, removes the icon and value when the icon is clicked', function () {
            var inputComp = utils_1.mountWithProvider(React.createElement(Input_1.default, { clearable: true }));
            var domNode = getInputDomNode(inputComp);
            setUserInputValue(inputComp, testValue); // user types into the input
            var iconComp = inputComp.find("Box[className~=\"" + Input_1.default.slotClassNames.icon + "\"]");
            ...
            expect(inputComp.find("Box[className~=\"" + Input_1.default.slotClassNames.icon + "\"]").length).toEqual(0); // the 'x' icon disappears
        });
    });
    it('disabled prop makes the input un-actionable', function () {
        var inputComp = utils_1.mountWithProvider(React.createElement(Input_1.default, { disabled: true }));
        var domNode = getInputDomNode(inputComp);
