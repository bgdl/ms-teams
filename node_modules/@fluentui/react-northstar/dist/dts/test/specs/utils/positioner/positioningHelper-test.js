...
describe('positioningHelper', function () {
    ...
    describe('positioningHelper offset argument transformed correctly in RTL', function () {
        it("applies transform only for 'above' and 'below' postioning", function () {
            var originalOffsetValue = '100%';
            expect(positioningHelper_1.applyRtlToOffset(originalOffsetValue, 'above')).not.toBe(originalOffsetValue);
            expect(positioningHelper_1.applyRtlToOffset(originalOffsetValue, 'below')).not.toBe(originalOffsetValue);
            expect(positioningHelper_1.applyRtlToOffset(originalOffsetValue, 'before')).toBe(originalOffsetValue);
            expect(positioningHelper_1.applyRtlToOffset(originalOffsetValue, 'after')).toBe(originalOffsetValue);
        });
        var expectOffsetTransformResult = function (originalOffset, resultOffset) {
            expect(positioningHelper_1.applyRtlToOffset(originalOffset, 'above')).toBe(resultOffset);
        };
        it('flips sign of simple expressions', function () {
            expectOffsetTransformResult('100%', '-100%');
            expectOffsetTransformResult('  2000%p ', '-2000%p');
            expectOffsetTransformResult('100  ', '-100');
            expectOffsetTransformResult(' - 200vh', '200vh');
        });
        it('flips sign of complex expressions', function () {
            expectOffsetTransformResult('100% + 200', '-100% - 200');
            expectOffsetTransformResult(' - 2000%p - 400 +800vh ', '2000%p + 400 -800vh');
        });
        it('transforms only horizontal offset value', function () {
            var xOffset = '-100%';
            var yOffset = '800vh';
            var offsetValue = [xOffset, yOffset].join(',');
            var _a = positioningHelper_1.applyRtlToOffset(offsetValue, 'above').split(','), xOffsetTransformed = _a[0], yOffsetTransformed = _a[1];
